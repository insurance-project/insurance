		...
		<p class="small text-center">접수 신청할 보험 선택하세요.</p>
			<table class="table table-bordered">
				<thead>
				  ...
				  <!-- insurance와 contract를 join 해서 contract의 judge가 true인 해당 고객의 보험만 출력해야 합니다. 매퍼 조건문에 "where contract.judge = 1 and contract.customerID = #{customerID}" 추가-->
				  <!-- 주소에 보험id 고객 id 다 넘겨줘야함: contract id 넘겨서 accidentReceipt 컨트롤러에서 contract id 통해서 보험/고객 id 받기..? 안되면 수정하셔도 됩니다-->
				  <tbody>
					 <c:forEach items="${contractList}" var="contractList" varStatus="status">
						<tr class="odd">
							<td><c:out value="${status.count}"/></td>
							<td><a href="accidnetReceipt?no=${contractList.getContractID()}">
