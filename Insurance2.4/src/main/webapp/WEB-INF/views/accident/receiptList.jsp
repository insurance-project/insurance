			...
			<table class="table table-bordered">
				<thead>
				    <tr>
				      ...
				      <th scope="col">보험 ID</th>
				      <th scope="col">사고 종류</th>
				      <th scope="col">사고 날짜</th>
				      <th scope="col">피해 금액</th>
				    </tr>
				  </thead>
				  <!-- accident, contract join? -->
				  <tbody>
					 <c:forEach items="${receiptList}" var="receiptList" varStatus="status">
						<tr class="odd">
							<td><c:out value="${status.count}"/></td>
							<td><a href="payment?no=${receiptList.getAccidentID()}">
								<c:out value="${receiptList.getCustomerID()}" /></a>
							</td>
							<td><a href="payment?no=${receiptList.getAccidentID()}">
								<c:out value="${receiptList.getInsuranceID()}" /></a>
							</td>
							<c:if test="${receiptList.getAccidentKind()=='TrafficAccident'}">
								<td>교통사고</td>
							</c:if>
							<c:if test="${receiptList.getAccidentKind()=='Fire'}">
								<td>화재 사고</td>
							</c:if>
							<c:if test="${receiptList.getAccidentKind()=='Wound'}">
								<td>상해</td>
							</c:if>
							<c:if test="${receiptList.getAccidentKind()=='Disease'}">
								<td>질병</td>
							</c:if>
