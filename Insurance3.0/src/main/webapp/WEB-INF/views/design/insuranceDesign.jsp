	...
	<div class="container w-25 mt-5">
		...
		<form action="insuranceDesign.do" class="mt-5" name="InsuranceDesign" method="POST">
			...
			<div class="mb-3">
				...
				<input type="text" class="form-control" name="Premium" placeholder="숫자만 입력하세요" onKeyPress="return checkNum(event)" onkeyup="return del(event)">
			</div>
			<div class="mb-3">
				<label for="price" class="form-label mb-2">보험 요율</label>
				<input type="text" class="form-control" name="PremiumRate" placeholder="숫자만 입력하세요" onKeyPress="return checkNum(event)" onkeyup="return del(event)">
			</div>
			
			<div class="mb-3">
				<label for="price" class="form-label mb-2">보장 기간</label>
				<input type="text" class="form-control" name="Warranty" placeholder="숫자만 입력하세요" onKeyPress="return checkNum(event)" onkeyup="return del(event)">
			</div>
			<div class="mb-3">
				<label for="price" class="form-label mb-2">최대 보상금</label>
				<input type="text" class="form-control" name="Reward" placeholder="숫자만 입력하세요" onKeyPress="return checkNum(event)" onkeyup="return del(event)">
			</div>
			
			<div class="mb-3">
				<label for="price" class="form-label mb-2">보상율</label>
				<input type="text" class="form-control" name="RewardRate" placeholder="숫자만 입력하세요" onKeyPress="return checkNum(event)" onkeyup="return del(event)">
			</div>
			
			<div class="text-center">
				<button type="submit" class="btn btn-primary" name="SubmitBtn" onclick="return design()">손익 예측 확인</button>
