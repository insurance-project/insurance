	...
	<div class="container w-25 mt-5">
		...
		<form action="insuranceModify.do" class="mt-5" name="InsuranceModify" method="POST">
			...
			<div class="mb-3">
				...
				<input type="text" class="form-control" name="Premium" placeholder="숫자만 입력하세요" value="${Premium}" onKeyPress="return checkNum(event)" onkeyup="return del(event)">
			</div>
			<div class="mb-3">
				<label for="price" class="form-label mb-2">보험 요율</label>
				<input type="text" class="form-control" name="PremiumRate" placeholder="숫자만 입력하세요" value="${PremiumRate}" onKeyPress="return checkNum(event)" onkeyup="return del(event)">
			</div>
			
			<div class="mb-3">
				<label for="price" class="form-label mb-2">보장 기간</label>
				<input type="text" class="form-control" name="Warranty" placeholder="숫자만 입력하세요" value="${Warranty}" onKeyPress="return checkNum(event)" onkeyup="return del(event)">
			</div>
			<div class="mb-3">
				<label for="price" class="form-label mb-2">최대 보상금</label>
				<input type="text" class="form-control" name="Reward" placeholder="숫자만 입력하세요" value="${Reward}" onKeyPress="return checkNum(event)" onkeyup="return del(event)">
			</div>
			
			<div class="mb-3">
				<label for="price" class="form-label mb-2">보상율</label>
				<input type="text" class="form-control" name="RewardRate" placeholder="숫자만 입력하세요" value="${RewardRate}" onKeyPress="return checkNum(event)" onkeyup="return del(event)">
			</div>
			
			<div class="text-center">
				<button type="submit" class="btn btn-primary" name="ModifyBtn" onclick="return modify()">수정 완료</button>
				<a href="deleteInsurance?insuranceID=${InsuranceID}"><button type="button" class="btn btn-danger" name="DeleteBtn" onclick="return deleteBtn()">삭제</button></a>
